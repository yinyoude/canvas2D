...
/* 
    接口拓展和类拓展一样，都是使用 extends 关键字
    类实现接口，则使用 implements 关键字
    注意， IEnumerator 中的泛型在 extends 时替换成了 IDoom3Token 类型
*/
export interface IDoom3Tokenizer extends IEnumerator < IDoom3Token > {
    setSource ( source : string ) : void ;
    
    // 一旦使用迭代器模式，实际上生产 IDoom3Token 的方法就不需要了
    /* createIDoom3Token ( ) : IDoom3Token; */
    // reset 方法已经定义在 IEnumerator 接口中了，不需要再在子接口中定义
    /* reset ( ) : void ; */
    // getNextToken 被 IEnumerator 的 moveNext 和 current 替代，因此在此接口中可以取消 
...
class Doom3Tokenizer implements IDoom3Tokenizer {
    ...
    // 增加一个私有变量 _current，并使用 new 进行初始化接口
    private _current : IDoom3Token = new Doom3Token ( ) ;

    // 实现 moveNext 方法，实际调用的是 getNextToken 方法
    public moveNext() : boolean {
        return this._getNextToken ( this . _current );  
    }

    // 通过 get 方式实现只读属性 current
